// Resend email integration for MK Markets
...
export async function sendVerificationEmail(toEmail: string, verificationToken: string): Promise<boolean> {
  try {
    ...
    const { data, error } = await client.emails.send({
      from: fromEmail || 'MK Markets <onboarding@resend.dev>',
      to: toEmail,
      subject: 'Verify your MK Markets account',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #2563eb; margin: 0;">MK Markets</h1>
            <p style="color: #666; margin-top: 5px;">Menlo School Edition</p>
          </div>
          
          <div style="background: #f8fafc; border-radius: 8px; padding: 30px; margin-bottom: 20px;">
            <h2 style="margin-top: 0;">Welcome to MK Markets!</h2>
          ...
          <div style="text-align: center; font-size: 12px; color: #999;">
            ...
            <p style="margin-top: 20px;">
              <strong>Reminder:</strong> MK Markets uses fake money only for educational purposes.<br>
...
export async function sendPasswordResetEmail(toEmail: string, resetToken: string): Promise<boolean> {
  try {
    ...
    const { data, error } = await client.emails.send({
      from: fromEmail || 'MK Markets <onboarding@resend.dev>',
      to: toEmail,
      subject: 'Reset your MK Markets password',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #2563eb;">MK Markets</h1>
