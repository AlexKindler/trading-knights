...
import { 
  Search, Zap, TrendingUp, TrendingDown, Clock, Activity, 
...
export default function Trading() {
  ...
  const movers = stocks?.slice().sort((a, b) => Math.abs(getChange(b)) - Math.abs(getChange(a))).slice(0, 6);
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-muted-foreground flex items-center gap-2" data-testid="text-page-subtitle">
              ...
              Real-time trading. Execute fast. Capitalize on momentum.
            </p>
          </div>
            ...
            {user?.status === "VERIFIED" && selectedStock ? (
              <>
                <Card className="border-primary/30">
                  <CardHeader className="pb-2">
                    ...
                    <p className="text-sm text-muted-foreground">{selectedStock.title}</p>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <div className={`text-sm ${getChange(selectedStock) >= 0 ? "text-green-600" : "text-red-600"}`}>
                          {getChange(selectedStock) >= 0 ? "+" : ""}{getChange(selectedStock).toFixed(2)}% from open
                        </div>
                      </div>
              ...
              <Card className="border-dashed">
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Click on any stock to start trading instantly
                  </p>
                </CardContent>
              </Card>
            ) : (
              <Card>
                <CardContent className="py-8 text-center">
                  <p className="text-muted-foreground mb-4">Sign in to start day trading</p>
                  <Link href="/login">
                    <Button data-testid="button-login">Login to Trade</Button>
                  </Link>
                </CardContent>
              </Card>
            )}
